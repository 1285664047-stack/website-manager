...
import urllib.request, urllib.error, urllib.parse, json, sys, time, pathlib, argparse
...
def main():
    ...
    if not API_KEY:
        ...
        print('  Set: python config_manager.py --key <your_api_key>')
        sys.exit(2)
    ...
    # Step 0: Save site info to config.json for persistence
    print('Step 0: Saving site info to config.json...')
    site_info = {k: v for k, v in company_data.items()}
    if config_manager.set_site_info_dict(site_info):
        print('  [OK] Site info saved to config.json')
    else:
        print('  [WARN] Failed to save site info to config.json, continuing...')
    time.sleep(0.5)
    ...
    if result.get('code') == 0 and result.get('data', {}).get('share_url'):
        share_url_path = result['data']['share_url']
        # API 返回相对路径（如 /api/template/preview/share/token/xxx），需提取 origin 拼接
