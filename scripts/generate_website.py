...
from config_manager import get_site_info, set_site_info_dict, reset_site_info
...
def do_initialize(state, label):
    """执行初始化并继续"""
    # 重置配置文件中的site_info（重新生成网站时清除原有企业信息）
    print("[INFO] 正在重置配置文件中的企业信息...")
    reset_site_info()
...
def do_generate(state):
    ...
    # 将收集到的企业信息存储到配置文件
    print("[INFO] 正在保存企业信息到配置文件...")
    set_site_info_dict(info)
    ...
    if result.get("code") == 0:
        ...
        if share_result and share_result.get("code") == 0 and share_result.get("data"):
            share_url_path = share_result.get("data", {}).get("share_url", "")
            if share_url_path:
                # API 返回相对路径，需提取 origin 拼接
                if not share_url_path.startswith('http'):
...
命令列表：
  ...
  python generate_website.py direct-generate   # 【快速】直接生成，绕过交互
    ...
    if not API_KEY:
        print(json.dumps({"error": "API key not set. Please run: python config_manager.py --key <your_api_key>"}))
        sys.exit(2)
